  ...
  var PAGES = {
    ...
    'priv-all': { crumb:'因私报告_因私出行', render:function(){
      ...
      return '<div class="callout danger">🔒 <b>因私报告</b>为消费者隐私数据专项报告：仅展示<b>聚合指标</b>，不可下钻、无组织/个人维度拆分、不提供明细导出。<b>整张报告可见性独立配置</b>（建议仅高管 / 合规角色可见），与各消费报告权限解耦——开通本报告即可见全部业务线因私聚合指标；不开通则各消费报告内均不再出现因私模块。</div>'
        ...
        + '<div class="hint">按月汇总因私出行全部指标：消费金额（占比 / 同比 / 环比）、使用人数（含同比）、产品/订单数（含同比）、均价（含同比）。聚合口径，不下钻、不拆组织/个人；合计与上方总览一致。</div>'
        + tableW(thead(['月份','因私消费金额','消费占比','同比','环比','使用人数（人次）','同比','产品/订单数','同比','均价','同比'])
            + trow(['2026-1','42,000元','18.1%',yoy(9.5),'—','33',yoy(4.0),'168',yoy(7.5),'250元',yoy(1.5)])
            + trow(['2026-2','38,000元','16.4%',yoy(12.0),yoy(-9.5),'29',yoy(6.5),'152',yoy(9.0),'250元',yoy(2.0)])
            + trow(['2026-3','41,000元','17.7%',yoy(14.2),yoy(7.9),'32',yoy(8.2),'166',yoy(11.2),'247元',yoy(2.8)])
            + trow(['2026-4','37,000元','15.9%',yoy(10.1),yoy(-9.8),'28',yoy(5.0),'150',yoy(8.0),'247元',yoy(1.2)])
            + trow(['2026-5','39,000元','16.8%',yoy(13.0),yoy(5.4),'34',yoy(9.1),'158',yoy(10.5),'247元',yoy(1.8)])
            + trow(['2026-6','35,200元','15.1%',yoy(11.5),yoy(-9.7),'30',yoy(7.0),'140',yoy(8.5),'251元',yoy(2.2)])
            + trow(['合计','232,200元','100%',yoy(11.8),yoy(0.8),'186 人次',yoy(6.8),'934',yoy(9.1),'—','—'], true))
